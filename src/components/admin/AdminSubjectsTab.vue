...
import { getSubjects, createSubject } from '../../services/admin'
...
// Backend schema: subjects { id, name }
const COLS = [
...
async function load() {
  loading.value = true
  try {
    subjects.value = (await getSubjects()).data ?? []
  } catch (e) {
...
async function submit() {
  ...
  try {
    await createSubject({ name: form.value.name })
    await load()
    modal.value = false
  } catch (e) {
...
<template>
  <div>
    <div class="tab-bar">
      <div>
        <p class="tab-title">Subjects</p>
        <p class="tab-sub">Define subjects taught across classes</p>
      </div>
      <button class="btn btn-ghost" @click="openModal">+ Add Subject</button>
    </div>
    ...
    <BaseModal title="New Subject" :show="modal" @close="modal = false">
